                ...
                return (
                  <div class="listing-box" key={index}>
                    <div className="close-add-buttons">
                      {counter < 5 ?
                        <img src={add} alt="Add Section" onClick={addSection} />
                        : null
                      }
                      {counter > 1 ?
                        <img src={trash} alt="Delete Section" onClick={() => deleteSection(index)} />
                        : null
                      }
                    </div>
                    <h3 onClick={() => setshowbg(false, '', '', 'true')}>
                      <input type="text" onChange={updateTile(index)} value={data.title} style={{ 'color': props.setTitleColor }} className={'child-container form-control border  size-auto ' + props.setAlignment + ' ' + props.setTextTransform} /></h3>
                    <div class="title" onClick={() => setshowbg(false, '', '', false)} >
                      <input type="text" onChange={updateFieldChanged(index)} value={data.text} style={{ 'color': props.setColor }} className={'child-container form-control border  size-auto ' + props.setAlignment + ' ' + props.setTextTransform} />
                    </div>
                  </div>
                );
              })
