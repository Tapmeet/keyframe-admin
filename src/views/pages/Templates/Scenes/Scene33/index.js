...
import TemplateSceneEight from "../Scene8";
const TemplateSceneThirtyThree = (props) => {
  const [bottomData, setBottomData] = React.useState("");
  const [sceneOrder, setSceneOrder] = React.useState("");
  const [userId, setUserId] = React.useState("");
  const [blocks, setBlocks] = React.useState("");
  const match = useRouteMatch("/template/:templateId/33/:sceneId");
  const {
    params: { templateId },
  } = match;
  const {
...
export default TemplateSceneThirtyThree;
